...
def play(inp):
    ...
    supports, supported_by = supporters(bricks)
    for brick in supports:
        can_disintegrate = True
        for upper in supports[brick]:
            if len(supported_by[upper]) == 1:
...
assert play(example_input) == 5

real_input = open('inputs/day22.input.txt').read()
print(play(real_input)) # => 530
